...
namespace Strategy
{
	...
	public class GameController : MonoBehaviour {
		...
		private float spiritSpawnInterval = 2;
		private float ghostSpawnInterval = 2;
		...
		IEnumerator StartDay (int day, float delay)
		{
			...
			float duration = 0;
			Color startColor = SetupUI.GetComponent <Image>().color;
			Color endColor = startColor;
			endColor.a = 0;
			Color startColor2 = SetupUI.transform.Find ("Text").GetComponent <Text> ().color;
			Color endColor2 = startColor2;
			endColor2.a = 0;
			while (SetupUI.GetComponent <Image>().color.a > endColor.a + 0.1f)
			{
				SetupUI.GetComponent <Image>().color = Color.Lerp (SetupUI.GetComponent <Image>().color, endColor, Time.deltaTime * 2);
				SetupUI.transform.Find ("Text").GetComponent <Text> ().color = Color.Lerp (SetupUI.transform.Find ("Text").GetComponent <Text> ().color, endColor2, Time.deltaTime * 2);
				duration += Time.deltaTime;
				yield return new WaitForSeconds (Time.deltaTime);
			}
			SetupUI.SetActive (false);
			SetupUI.GetComponent <Image> ().color = startColor;
			SetupUI.transform.Find ("Text").GetComponent <Text> ().color = startColor2;
		}
		...
		public void EndDay (string tag, float delay)
		{
			if (!endDay)
			{
				endDay = true;
				StartCoroutine (EndDayDelay (tag, delay));
			}
		}

		IEnumerator EndDayDelay (string tag, float delay)
		{
			float duration = 0;
			Color startColor = curtain.GetComponent <Image>().color;
			Color endColor = curtain.GetComponent <Image>().color;
			startColor.a = 0;
			endColor.a = 1;
			if (!curtain.activeSelf)
			{
				curtain.GetComponent <Image>().color = startColor;
				curtain.SetActive (true);
			}
			while (duration < delay)
			{
				curtain.GetComponent <Image>().color = Color.Lerp (curtain.GetComponent <Image>().color, endColor, Time.deltaTime * 3);
				duration += Time.deltaTime;
				yield return new WaitForSeconds (Time.deltaTime);
			}
			EndDay (tag);
