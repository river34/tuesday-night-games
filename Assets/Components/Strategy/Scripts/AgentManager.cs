...
namespace Strategy
{
	public class AgentMeta {
		...
		public void LevelUp (ref AgentMeta meta)
		{
			...
			if (meta.tag == "Tree")
			{
				if (meta.health != -1) meta.health += 40;
			}
		...
		public void LevelUp (int _level)
		{
			...
			if (tag == "Tree")
			{
				if (health != -1) health += 40 * (_level - level);
			}
	...
	public class AgentManager : MonoBehaviour {
		...
		void Awake ()
		{
			if (agentHolder == null)
			{
				...
				agentHolder.transform.parent = transform.parent;
				agentHolder.transform.position = new Vector3 (0, 0, -1);
			}
